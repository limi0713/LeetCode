/*
 * @lc app=leetcode.cn id=5393 lang=golang
 *
 * [5393] 可获得的最大点数
 *
...
func maxScore(cardPoints []int, k int) int {
    if len(cardPoints) <= k{
        return sum(cardPoints,len(cardPoints))
    }
    ...
    for i := len(cardPoints) - k;i<len(cardPoints);i++ {
        ...
        curSum = curSum + cardPoints[i] - cardPoints[i-len(cardPoints)+k];
        if curSum < minSum {
            minSum = curSum
        }
    }
...
func sum(cardPoints []int,leng int) int {
