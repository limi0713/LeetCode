...
func removeComments(source []string) []string {
    ...
    for i:=0;i<len(source);i++{
        if ok {
            tag,code = handleStrWithBlock(source[i])
        }else{
            tag,code = handleStrWithoutBlock(source[i])
        }
...
func handleStrWithoutBlock(source string)(bool,string){
    for i:=0;i<len(source);i++{
        if i+1<len(source)&&source[i:i+2] == "//"{
            return false,source[0:i] 
        }

        if i+1<len(source)&&source[i:i+2] == "/*"{
            okT,resT := handleStrWithBlock(string(source[i+2:]))
            return okT,source[0:i]+resT
        }
    }
...
func handleStrWithBlock(source string)(bool,string){
    for i:=0;i<len(source);i++{
        if i+1<len(source) &&source[i:i+2] == "*/"{
            if i+2 < len(source){
                return handleStrWithoutBlock(string(source[i+2:]))
            }else{
                return false,""
            }
        }
